...
function checkVictory(i, j){
    ...
    for(let k = 1;k< 4;k++){
        if(j - k < 0){
            break  //count of the color towards left
        }
        if(board[i][j - k] !== c){
            break  //If color is not equal to original color
        }
        count++
    }
    ...
    for(let k = 1;k < 4; ++k){
        if(j + k > 7){
            break  //count of the color towards right
        }
        if(board[i][j + k] !== c){
            break  //if color is not equal to original color
        }
    ...
    //count upto three places left or right
    if(count > 2){
        return true
    }
...
if(players['red'] == null){
    ...
    socket.emit('color','yellow') //sends to paricular client
    io.emit('turn','red') //broadcast same message to all connected
}else{
    socket.disconnect()
}
...
socket.on('click', function(column){
    ...
    if(players[player] !== socket){
        console.log('click from wrong layer: '+player === 'red'?'yellow' : 'red')
        return
    }
